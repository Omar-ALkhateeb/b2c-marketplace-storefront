  ...
  return (
    <>
      {Object.keys(groupedItems).map((key) => (
        <div key={key} className="bg-white rounded-2xl shadow-sm overflow-hidden mb-3">
...
function groupItemsBySeller(cart: HttpTypes.StoreCart) {
  ...
  cart.items?.forEach((item: any) => {
    ...
    if (seller) {
      ...
      if (!groupedBySeller["kiddo"]) {
        groupedBySeller["kiddo"] = {
          seller: {
            name: "Kiddo",
            id: "kiddo",
            photo: "/Kiddo.png",
      ...
      groupedBySeller["kiddo"].items.push(item)
    }
  })
